...
AActor* AVehicleGameMode::ChoosePlayerStart_Implementation(AController* Player)
{
	...
	for (TActorIterator<APlayerStart> PlayerStartIt(GetWorld()); PlayerStartIt; ++PlayerStartIt)
	{
		...
		{
			if ((Pawn->GetActorLocation() - TestSpot->GetActorLocation()).SizeSquared2D() < 10000)
			{
				bBlocked = true;
				break;
			}
...
void AVehicleGameMode::Tick(float DeltaSeconds)
{
	...
	// Start race when running client/server
	// GetGameMode is not available to client in the HUD blueprint so
	// we need this workaround to get the race started on the server.
	if (!IsRaceActive() && GetWorld()->GetTimeSeconds() > 5.0f)
	{
		StartRace();
	}
}
