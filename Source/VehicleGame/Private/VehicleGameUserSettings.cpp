...
bool UVehicleGameUserSettings::IsMouseSensitivityDirty() const
{
	...
	if (GEngine)
	{
		...
		for (auto It = PlayerList.CreateIterator(); It; ++It)
		{
			...
			float CurrentMouseSensitivity = PC->PlayerInput->GetMouseSensitivityX();  // It's ok to use X here as we always set X and Y to the same values
...
void UVehicleGameUserSettings::ResetToCurrentSettings()
{
	...
	if (GEngine)
	{
		...
		for (auto It = PlayerList.CreateIterator(); It; ++It)
		{
			...
			SetMouseSensitivity(PC->PlayerInput->GetMouseSensitivityX()); // It's ok to use X here as we always set X and Y to the same values
